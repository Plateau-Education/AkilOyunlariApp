...
class HazineAvi:
    ...
    def solver2(self, grid):
        self.depth = 0
        for __ in range(self.boyut**2):
            op_list = self.first_base(grid)
            if op_list == "Wrong Question":
                return "Wrong Question"
            op_list = op_list.union(self.second_base(grid))
            # print(op_list)
            if len(op_list)>0:
                for op in op_list:
                    grid[op[0]][op[1]] = op[2]
...
def main(size, tkinterOn=False):
    ...
    for _ in range(10):
        soru = HazineAvi()
        soru.boyut = size
        a = soru.class_main(tkinterOn=tkinterOn)
        total_discarded+=soru.discarded
    print("TOTAL_DISCARDED: ",total_discarded)


# start = timeit.default_timer()
cozulmus = main(5,False)
# print(np.matrix(cozulmus))
# end = timeit.default_timer()
